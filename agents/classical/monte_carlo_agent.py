...
class MonteCarloAgent(BaseClassicalAgent):
    ...
    def update(
        ...
        if done:
            ...
            if episode_states:
                try:
                    avg_q = np.mean([np.max(self.q_table[s]) for s in episode_states if s in self.q_table])
    ...
    def train_episode(
        ...
        while not done and steps < max_steps:
            ...
            if verbose and steps % 100 == 0:
                print(f"Step {steps}: Reward={reward:.4f}, Portfolio={info.get('portfolio_value', 0):.2f}")
    ...
    def load(self, filepath: str):
        ...
        self.use_sample_average = data.get("use_sample_average", self.use_sample_average)
        ...
        print(f"  First-visit: {self.first_visit}, sample-average: {self.use_sample_average}")
